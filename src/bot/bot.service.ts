...
export class BotService {
  ...
  private async setupHandlers() {
    ...
    this.bot.action('show_guide_details', async (ctx: Context) => {
      const guideText = `ГАЙД «Профилактика дисфункций мышц тазового дна: советы на каждый день»
...
🎁 Сейчас цена всего 511 руб. (потом 911 руб.), а если оставишь отзыв — верну 10%!`;
      ...
      try {
        ...
        // Фолбэк: если фото не отправилось, отправляем только текст
        await ctx.reply(
          guideText,
          Markup.inlineKeyboard([
            [
